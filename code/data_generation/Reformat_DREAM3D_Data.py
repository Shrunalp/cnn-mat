...
### Reformat Data for Training ###
    ...
    X = np.load(FID_IPF_Data)
    Y = np.load(Phase_Quat_Data)
    ...
    for i in range(Block_Size[1]):
        for j in range(Block_Size[2]):
            for k in range(Block_Size[3]):
                img_data_FID[0][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][0]
                img_data_FID[1][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][0]
                img_data_FID[2][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][0]
                img_data_FID[3][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][0]
...
### RGB 2D Array using IPF Colors ###
    ...
    for i in range(Block_Size[1]):
        for j in range(Block_Size[2]):
            for k in range(Block_Size[3]):
                img_data_IPF[0][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][1]
                img_data_IPF[1][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][2]
                img_data_IPF[2][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][3]
                img_data_IPF[3][i][j][k] = X[i+Block_Size[2]*j+(Block_Size[1]*Block_Size[2])*k][1]
    ...
    for i in range(Block_Size[1]):
        for j in range(Block_Size[2]):
            for k in range(Block_Size[3]):
                ...
                img_data_PQ[0][i][j][k] = Y[x-1][1]
                img_data_PQ[1][i][j][k] = Y[x-1][2]
                img_data_PQ[2][i][j][k] = Y[x-1][3]
                img_data_PQ[3][i][j][k] = Y[x-1][4]

    print('Preprocessing is finished.')

### Save ###
    np.save("FID_"+FID_IPF_Data, img_data_FID)
    np.save("IPF_"+FID_IPF_Data, img_data_IPF)
    np.save("PQ_"+Phase_Quat_Data , img_data_PQ)


#####################################################################################################


def b2w(Block_data, ht):
